...
const CanceledBookingReportComponent1 = () => {
  const dispatch = useDispatch();
  const [filterForm] = Form.useForm();
  const [showFilter, setshowFilter] = useState(false);
  const [reports, setReports] = useState([]);
  const [ExportData, setExportData] = useState([]);
  const { canceledbookingrept1_list, isLoading } = useSelector(
    (State) => State.Reports
  );
  ...
  const onFormFilterLayoutChange = () => {
    ...
    if (agency !== undefined && agency !== "") {
      dispatch({
        type: actions.GET_CANCELEDBOOKINGREPT1_LIST,
        payload: {
          agency: agency,
          fdate:
            fromdate !== undefined
  ...
  useEffect(() => {
    canceledbookingrept1_list.length > 0 &&
      setReports(canceledbookingrept1_list);
          ...
          <Col>
            <Space>
              {ExportData.length > 0 && (
                <MyCsvLink
                  csvData={ExportData}
                  filename="Canceled_Booking_Report1"
                />
              )}
...
export default CanceledBookingReportComponent1;
