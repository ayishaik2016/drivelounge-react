...
import { Col, Button, Typography, message } from "antd";
...
const CarItem = (props) => {
  ...
  const [ChangeColor, setChangeColor] = useState(isfavorites ? "red" : "black");
  ...
  const handleFavorite = () => {
    ...
    if (ChangeColor === "black") {
      ...
      dispatch({
        ...
        callBackAction: () => {
          message.success(getLocaleMessages("Added to your favorite list!"));
        },
      });
    } else {
  ...
  const handleRentNow = () => {
    ...
    if (localValue.length == 0 || localValue["PickPlace"] == "") {
      message.error(
        getLocaleMessages(
          "Please select the pick up and drop place to proceed."
        )
      );
      return;
    }
    ...
    localStorage.setItem("caridl", JSON.stringify(carid));
    dispatch({
  ...
  useEffect(() => {
    dispatch({
      type: carActions.GET_CAR_INTERIOR_LIST,
      payload: carid,
    });
  }, [carid]);
            ...
            <div className="img">
              {getLocalDataType() == "user" && (
                <div className="fav" onClick={handleFavorite}>
                  <HeartIcon style={{ color: ChangeColor }} />
                </div>
              )}
            ...
            <div className="specification">
              ...
              <div className="foroneDays">
                <Paragraph>
                  ...
                  <span className="bold">
                    {symbol
                      ? `${preferredCurrency} ${(carpriceperday * parseFloat(currencyConversion).toFixed(2)).toFixed(decimal)}`
                      : `${Formatcurrency((carpriceperday * parseFloat(currencyConversion).toFixed(2)).toFixed(decimal))} ${preferredCurrency}`}
                  </span>
                </Paragraph>
